...
async def optimize_sermon(
    ...
    if file_record.user_id != current_user.id:
        if "files:view_all" not in [p.name for r in current_user.roles for p in r.permissions]:
...
async def get_sermon_team(
    ...
    return {
        "file_id": file_id,
        "team": metadata.get("assigned_team", {})
    }
...
async def batch_optimize(
    ...
    for file_id in file_ids:
        ...
        if file_record.user_id != current_user.id:
            results.append({"file_id": file_id, "success": False, "error": "Access denied"})
            continue
        ...
        if result:
            ...
            results.append({"file_id": file_id, "success": False, "error": "Optimization failed"})
