...
class ResourceOwnerChecker:
    ...
    def __init__(
        self, resource_id_param: str, resource_table, owner_field: str = "user_id"
    ):
...
def filter_query_by_role(
    db: Session, user: User, query, resource_table, owner_field: str = "user_id"
):
    ...
    return query.filter(getattr(resource_table, owner_field) == user.id)
...
async def refresh_token(refresh_token: str, db: Session) -> Optional[dict]:
    ...
    new_token = create_access_token(data={"sub": str(user.id)}, db=db, user_id=user.id)
