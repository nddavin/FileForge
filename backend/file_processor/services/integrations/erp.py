...
class SAPConnector(IntegrationBase):
    ...
    def send(
        self, endpoint: str, data: Dict[str, Any], method: str = "GET"
    ) -> IntegrationResult:
        """Send data to SAP"""
        try:
    ...
    def create_material(self, material_data: Dict[str, Any]) -> IntegrationResult:
        ...
        return self.send(
            "/sap/opu/odata/sap/API_PRODUCT_SRV/A_Product", material_data, "POST"
        )
    ...
    def upload_document(self, doc_data: Dict[str, Any]) -> IntegrationResult:
        ...
        return self.send(
            "/sap/opu/odata/sap/API_DOCUMENT_SRV/A_Document", doc_data, "POST"
        )
    ...
    def _authenticate(self) -> IntegrationResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return IntegrationResult(
                    success=True, data={"csrf_token": self._csrf_token}
                )
            else:
                return IntegrationResult(
                    success=False, error=f"CSRF token fetch failed: {response.text}"
                )
        except Exception as e:
            return IntegrationResult(success=False, error=str(e))
...
class OracleERPConnector(IntegrationBase):
    ...
    def create_supplier(self, supplier_data: Dict[str, Any]) -> IntegrationResult:
        ...
        return self.send(
            "/fscmRestApi/resources/11.13.18.05/suppliers", supplier_data, "POST"
        )

    def create_invoice(self, invoice_data: Dict[str, Any]) -> IntegrationResult:
        """Create an invoice in Oracle ERP"""
        return self.send(
            "/fscmRestApi/resources/11.13.18.05/invoices", invoice_data, "POST"
        )

    def upload_document(self, doc_data: Dict[str, Any]) -> IntegrationResult:
        """Upload a document to Oracle ERP"""
        return self.send(
            "/fscmRestApi/resources/11.13.18.05/documents", doc_data, "POST"
        )
    ...
    def _authenticate(self) -> IntegrationResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return IntegrationResult(success=True, data=token_data)
            else:
                return IntegrationResult(
                    success=False, error=f"Authentication failed: {response.text}"
                )
        except Exception as e:
            return IntegrationResult(success=False, error=str(e))
