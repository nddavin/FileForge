...
class DocuSignConnector(IntegrationBase):
    ...
    def get_signed_document(
        self, envelope_id: str, document_id: str = "1"
    ) -> IntegrationResult:
        ...
        try:
            ...
            full_url = f"{self.config.base_url}/v2.1/accounts/{self._account_id}{url}"
            headers = self._get_headers()
            headers["Authorization"] = f"Bearer {self._access_token}"
    ...
    def void_envelope(self, envelope_id: str, void_reason: str) -> IntegrationResult:
        """Void an envelope"""
        return self.send(
    ...
    def list_envelopes(
        self, from_date: Optional[str] = None, status: Optional[str] = None
    ) -> IntegrationResult:
