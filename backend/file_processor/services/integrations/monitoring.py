...
class AnomalyDetection:
    ...
    algorithms: List[str] = field(default_factory=lambda: ["zscore", "iqr", "isolation_forest"])
        ...
        with self._lock:
            for alert in self._alerts.values():
                if alert.rule_id == rule_id and alert.status not in [
                    ...
                    if (datetime.now(timezone.utc) - started).total_seconds() < rule.cooldown_seconds:
                        return None
...
class MetricsService:
    ...
    def get_metric_values(
        ...
        cutoff = datetime.fromisoformat(since) if since else (
...
class CapacityPlanningService:
    ...
    def get_capacity_trend(
        ...
        with self._lock:
            samples = [
                getattr(m, metric) for m in self._history
    ...
    def _generate_recommendations(self, forecasts: Dict[str, float]) -> List[str]:
        ...
        if forecasts.get("cpu_usage_percent", 0) > 80:
            recommendations.append("Consider scaling up CPU resources or optimizing workloads")
        
        if forecasts.get("memory_usage_percent", 0) > 85:
            recommendations.append("Memory usage is high - consider adding RAM or optimizing memory usage")
        
        if forecasts.get("disk_usage_percent", 0) > 80:
            recommendations.append("Disk space is filling up - consider archival or expansion")
    ...
    def get_capacity_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "trends": {
                ...
                for m in ["cpu_usage_percent", "memory_usage_percent", "error_rate_percent"]
            },
            "forecast": forecast,
    ...
    def get_cost_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "budget": {
                ...
                "status": "over_budget" if summary.get("budget_utilization", 0) > 100 else "under_budget"
            }
        }
...
class AnomalyDetectionService:
    ...
    def add_training_data(
        ...
        with self._lock:
            ...
            self._training_data[key].append({
                "value": value,
                "timestamp": timestamp
            })
    ...
    def _run_detection_algorithm(
        ...
        if algorithm == "zscore":
            ...
            return self._isolation_forest_detection(historical, current_value, metric_name, key)
        elif algorithm == "exponential_smoothing":
            return self._exp_smoothing_detection(historical, current_value, metric_name, key)
    ...
    def _zscore_detection(
        ...
        if zscore > (3 - self._config.sensitivity * 2):
            deviation = ((current_value - mean_val) / mean_val * 100) if mean_val != 0 else 0
    ...
    def _iqr_detection(
        ...
        if current_value < lower_bound or current_value > upper_bound:
            deviation = ((current_value - statistics.mean(historical)) / statistics.mean(historical) * 100) if statistics.mean(historical) != 0 else 0
    ...
    def _isolation_forest_detection(
        ...
        if isolation_score > (0.7 - self._config.sensitivity * 0.3):
            ...
            return DetectedAnomaly(
                ...
                metric_labels={},
                anomaly_type="outlier",
                severity=AlertSeverity.MEDIUM if isolation_score < 0.9 else AlertSeverity.HIGH,
    ...
    def _exp_smoothing_detection(
        ...
        recent_errors = [abs(historical[i] - historical[i-1]) for i in range(1, len(historical))]
        avg_error = statistics.mean(recent_errors) if recent_errors else 1
        
        if error > avg_error * (3 - self._config.sensitivity * 2):
            deviation = ((current_value - smoothed) / smoothed * 100) if smoothed != 0 else 0
...
class MonitoringOrchestrator:
    ...
    def _collect_system_metrics(self):
        ...
        # Update capacity
        self.capacity.record_capacity_metrics(CapacityMetrics(
    ...
    def get_full_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            "alerts": {
                ...
                "critical": len(self.alerting.get_active_alerts(AlertSeverity.CRITICAL)),
