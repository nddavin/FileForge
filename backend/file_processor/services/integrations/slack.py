...
class SlackConnector(IntegrationBase):
    ...
    def upload_file(
        ...
        try:
            ...
            files = {"file": (filename, file_content, filetype or "auto")}
    ...
    def schedule_message(
        self, channel: str, text: str, post_at: int  # Unix timestamp
    ) -> IntegrationResult:
        """Schedule a message for later delivery"""
        data = {"channel": channel, "text": text, "post_at": post_at}
    ...
    def _authenticate(self) -> IntegrationResult:
        ...
        if not self._bot_token:
            return IntegrationResult(success=False, error="Bot token is required")

        return IntegrationResult(success=True, data={"authenticated": True})

    @classmethod
    def create_config(
        cls, bot_token: str, signing_secret: Optional[str] = None
    ) -> IntegrationConfig:
