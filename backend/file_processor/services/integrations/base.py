...
class IntegrationConfig:
    ...
    def __post_init__(self):
        ...
        if (
            self.auth_type == AuthenticationType.API_KEY
            and "api_key" not in self.credentials
        ):
...
class IntegrationBase(ABC):
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        if self.config.auth_type == AuthenticationType.API_KEY:
            headers["Authorization"] = (
                f"ApiKey {self.config.credentials.get('api_key', '')}"
            )
        elif self.config.auth_type == AuthenticationType.BEARER_TOKEN:
            headers["Authorization"] = (
                f"Bearer {self.config.credentials.get('token', '')}"
            )
    ...
    def _log_request(self, method: str, url: str, data: Dict[str, Any]):
        ...
        logger.info(
            f"[{self.integration_name}] {method} {url} - Data: {json.dumps(safe_data)}"
        )
    ...
    def _log_result(self, result: IntegrationResult):
        ...
        if result.success:
            logger.info(
                f"[{self.integration_name}] Success - Status: {result.status_code}"
            )
        else:
