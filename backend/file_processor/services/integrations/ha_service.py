...
class ClusterNode:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            and self.cpu_usage < 90
            and self.memory_usage < 90
        )
...
class HAClusterService:
    ...
    def add_node(self, node: ClusterNode) -> bool:
        ...
        with self._lock:
            ...
            self._log_event(
                "node_joined", {"node_id": node.node_id, "region": node.region}
            )
    ...
    def start(self):
        ...
        logger.info(
            f"HA cluster {self.cluster_id} started with {len(self._nodes)} nodes"
        )
    ...
    def get_node_for_request(
        self, prefer_region: Optional[str] = None, prefer_local: bool = True
    ...
    def force_failover(self, target_node_id: Optional[str] = None) -> bool:
        ...
        with self._lock:
            ...
            if target_node_id:
                ...
                standbys = [
                    n for n in self._nodes.values() if n.state == NodeState.STANDBY
                ]
                if not standbys:
                    return False
            ...
            logger.info(
                f"Failover completed: {old_leader} -> {target.node_id} ({failover_duration:.2f}ms)"
            )
            return True
    ...
    def get_cluster_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            failed_nodes = [
                n for n in self._nodes.values() if n.state == NodeState.FAILED
            ]
        ...
        with self._lock:
            return {
                ...
                "health_check_success_rate": (
                    self._metrics.successful_health_checks
                    / self._metrics.node_health_checks
                    if self._metrics.node_health_checks > 0
                    else 1.0
                ),
                "node_health": {
    ...
    def _promote_to_active(self, node_id: str):
        ...
        self._log_event("node_promoted", {"from": old_leader, "to": node_id})

    def _check_quorum(self) -> bool:
        """Check if cluster has quorum"""
        active_count = sum(
            1 for n in self._nodes.values() if n.state == NodeState.ACTIVE
        )
                ...
                with self._lock:
                    ...
                    for node_id, node in list(self._nodes.items()):
                        ...
                        try:
                            ...
                            if (
                                elapsed
                                > self.failover_policy.health_check_interval_seconds * 3
                            ):
                    ...
                    for i, item in enumerate(self._replication_queue):
                        ...
                        for node in healthy_nodes[:target_count]:
                            try:
                                ...
                                logger.warning(
                                    f"Replication to {node.node_id} failed: {e}"
                                )
...
class DnsFailoverService:
    ...
    def add_record(
        self, hostname: str, targets: List[Dict[str, Any]], health_check_url: str
    ):
        ...
        with self._lock:
            return {
                "records": {
                    hostname: {
                        "current_ip": (
                            record["current_target"]["ip"]
                            if record["current_target"]
                            else None
                        ),
                        "region": (
                            record["current_target"]["region"]
                            if record["current_target"]
                            else None
                        ),
