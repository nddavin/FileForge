...
class Endpoint:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.health == HealthStatus.HEALTHY and
...
class LoadBalancer:
    ...
    def mark_failure(self, endpoint: Endpoint):
        ...
        endpoint.health = HealthStatus.DEGRADED if endpoint.failure_count < 5 else HealthStatus.UNHEALTHY
    
    def mark_success(self, endpoint: Endpoint, latency_ms: float):
        """Mark an endpoint as successful"""
        # Update moving average latency
        endpoint.latency_avg_ms = (
            (endpoint.latency_avg_ms * 0.7) + (latency_ms * 0.3)
        )
    ...
    def _execute_with_ha(
        ...
        while attempt < (self._ha_config.max_retries if self._ha_config else 3):
            ...
            try:
                ...
                logger.warning(
                    f"Operation {operation} failed (attempt {attempt}): {e}"
                )
...
class ClusterManager:
    ...
    def _check_failover(self):
        ...
        healthy_regions = [
            r for r in self.config.failover_regions
