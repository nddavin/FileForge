...
class SharingService:
    ...
    def _hash_password(self, password: str) -> str:
        ...
        return hashlib.pbkdf2_hmac(
            "sha256", password.encode(), self._secret_key.encode(), 100000
        ).hex()
...
class VersioningService:
    ...
    def rollback(
        self, file_id: str, version_number: int, user_id: str
    ) -> Dict[str, Any]:
...
class CollaborationService:
    ...
    def create_session(self, file_id: str, created_by: str) -> CollaborationSession:
    ...
    def update_cursor(self, session_id: str, user_id: str, position: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self._notify_participants(
                session_id, "cursor_update", {"user_id": user_id, "position": position}
            )
    ...
    def remove_reaction(self, comment_id: str, user_id: str, emoji: str) -> bool:
        ...
        with self._lock:
            for comments in self._comments.values():
                for comment in comments:
                    if comment.comment_id == comment_id:
                        if (
                            emoji in comment.reactions
                            and user_id in comment.reactions[emoji]
                        ):
        ...
        with self._lock:
            ...
            return {
                ...
                "recent_activities": [
                    ...
                    for c in sorted(comments, key=lambda x: x.created_at, reverse=True)[
                        :5
                    ]
                ],
            }
