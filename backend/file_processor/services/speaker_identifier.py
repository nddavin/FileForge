...
class SermonSpeakerIdentifier:
    ...
    async def identify_speakers(
        ...
        try:
            ...
            for turn, _, speaker_label in turns:
                segment_audio = await self._extract_segment(
            ...
            result.speaker_stats = self._calculate_speaker_stats(
                result.segments, church_team
            )
            ...
            result.total_speakers = len(
                set(s.speaker_id for s in result.segments if s.speaker_id)
            )
    ...
    def _match_speaker(
        self, test_embedding: np.ndarray, known_embeddings: Dict[str, List[float]]
    ) -> Dict[str, Any]:
    ...
    async def _extract_segment(self, audio_path: str, start: float, end: float) -> str:
    ...
    def _get_primary_speaker(
        self, segments: List[SpeakerSegment], church_team: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _calculate_speaker_stats(
        ...
        name_map = {
            m.get("id"): m.get("full_name") or m.get("name") for m in church_team
        }
        ...
        for segment in segments:
            ...
            speaker_confidences[sid] = speaker_confidences.get(sid, []) + [
                segment.confidence
            ]
...
class VoiceprintManager:
    ...
    async def register_preacher_voiceprint(
        self, preacher_id: str, audio_paths: List[str]
    ) -> Dict[str, Any]:
    ...
    async def load_voiceprints(self, church_id: str) -> Dict[str, List[float]]:
        ...
        try:
            result = (
                supabase.table("preacher_voiceprints")
                .select("preacher_id, embedding")
                .execute()
            )
...
async def identify_sermon_speakers(
    audio_path: str, church_id: str, supabase_client=None
) -> SpeakerIdentificationResult:
    ...
    if supabase_client:
        try:
            ...
            # Run identification
            return await identifier.identify_speakers(
                audio_path, church_team, voiceprints
            )
