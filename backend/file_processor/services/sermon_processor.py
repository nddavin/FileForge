...
class LocationDetector:
    ...
    async def extract_location(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            if coordinates:
                return await self._reverse_geocode(coordinates["lat"], coordinates["lon"])
...
class MultiModalDetector:
    ...
    def detect_components(self, file_paths: List[str]) -> Dict[str, bool]:
        ...
        return {
            "has_video": any(
                Path(f).suffix.lower() in self.VIDEO_FORMATS
                for f in file_paths
            ),
            "has_audio": any(
                Path(f).suffix.lower() in self.AUDIO_FORMATS
                for f in file_paths
            ),
            "has_transcript": any(
                Path(f).suffix.lower() in self.TRANSCRIPT_FORMATS
                for f in file_paths
            ),
            "has_artwork": any(
                Path(f).suffix.lower() in self.ARTWORK_FORMATS
                for f in file_paths
            )
        }
        ...
        if self.supabase:
            try:
                result = await self.supabase.db.select(
                    "profiles",
                    {"filters": {"church_id": church_id}, "limit": 10}
                )
        ...
        try:
            ...
            for stream in data.get("streams", []):
                if stream.get("codec_type") == "video":
                    metrics.video_resolution = f"{stream.get('width', 0)}x{stream.get('height', 0)}"
                    ...
                    if "/" in r_frame_rate:
                        ...
                        metrics.video_frame_rate = float(num) / float(den) if float(den) else 0
    ...
    async def process_sermon(
        ...
        for path in file_paths:
            if Path(path).suffix.lower() in self.modal_detector.VIDEO_FORMATS.union(
                ...
                results["metadata"].duration_seconds = self.modal_detector.get_duration(path)
                break
        ...
        for path in file_paths:
            if Path(path).suffix.lower() in self.modal_detector.TRANSCRIPT_FORMATS:
                ...
                if analysis:
                    results["metadata"].series_title = analysis.get("series_title") or series_title
                    results["metadata"].sermon_title = analysis.get("sermon_title")
                    results["metadata"].theme_scripture = analysis.get("theme_scripture")
