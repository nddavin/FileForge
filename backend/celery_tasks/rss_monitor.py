...
MEDIA_TYPES = {
    MediaType.AUDIO: [
        ...
        'audio/aac', 'audio/ogg', 'audio/mp3', 'audio/x-m4a'
    ],
    MediaType.VIDEO: [
        'video/mp4', 'video/quicktime', 'video/x-msvideo', 
        'video/m4v', 'video/webm', 'video/x-matroska'
    ],
    MediaType.IMAGE: [
...
class EnhancedRSSMonitor:
    ...
    def process_entry(self, entry, feed_title: str) -> Optional[Dict]:
        ...
        if supabase:
            existing = supabase.table('sermon_packages').select('id').eq(
                'rss_guid', guid
            ).execute()
            if existing.data:
                return None
        ...
        has_media = (
            len(media_groups[MediaType.VIDEO.value]) > 0 or
            len(media_groups[MediaType.AUDIO.value]) > 0
        )
...
def process_sermon_video_pipeline(self, package_id: str, primary_video: Dict, 
    ...
    try:
        ...
        from backend.celery_tasks.sermon_workflow import (
            extract_gps_location, analyze_sermon_metadata
        )
...
@shared_task(bind=True, max_retries=2)
def process_sermon_audio_pipeline(self, package_id: str, primary_audio: Dict,
                                   all_audio: List[Dict]):
...
def process_sermon_images(self, package_id: str, images: List[Dict]):
    ...
    try:
        ...
        return {'status': 'images_complete', 'package_id': package_id, 'count': len(images)}
