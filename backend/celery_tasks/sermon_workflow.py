...
async def ai_assign_team(sermon_id: str, task_types: List[str]) -> Dict[str, str]:
    ...
    try:
        ...
        ).eq('role', 'media_team').eq('active', True).execute()
...
# ==================== Task Creation ====================
def create_sermon_tasks(sermon_id: str, assignments: Dict[str, str], 
    ...
    for spec in task_specs:
        ...
        try:
            result = supabase.table('sermon_tasks').insert({
                'sermon_id': sermon_id,
                'task_type': task_type,
                'status': 'assigned',
...
def sermon_intake_pipeline(self, sermon_id: str, uploaded_files: List[str],
    ...
    try:
        ...
        task_types = ['transcription', 'location_tagging', 'metadata_ai', 
                      'quality_optimization', 'thumbnail_generation']
        
        has_video = any(Path(f).suffix.lower() in ['.mp4', '.mov', '.mkv'] 
        ...
        gps_task = extract_gps_location.delay(
            sermon_id,
            assignments.get('location_tagging')
        )
        ...
        quality_task = optimize_quality.delay(
            sermon_id,
            assignments.get('quality_optimization')
        )
...
def transcribe_sermon(self, sermon_id: str, assigned_user: Optional[str] = None):
    ...
    try:
        ...
        transcript_record = supabase.table('sermon_transcripts').insert({
            'sermon_id': sermon_id,
        ...
        transcript_id = transcript_record.data[0]['id'] if transcript_record.data else None
        ...
        _handle_task_failure(supabase, sermon_id, 'transcription', e, self.request.retries)
        raise
...
def process_video(self, sermon_id: str, assigned_user: Optional[str] = None):
    ...
    try:
        ...
        from file_processor.services.sermon_processor import (
            SermonProcessor, OPTIMIZATION_PROFILES
        )
        ...
        if supabase:
            update_data = {
                'optimized_web_path': web_result.get('output_path') if web_result else None,
                'archive_path': archive_result.get('output_path') if archive_result else None
            }
            ...
            supabase.table('sermon_tasks').update({
                'status': 'completed',
                'completed_at': datetime.now(timezone.utc).isoformat(),
                'result_data': {'web_optimized': web_result, 'archive': archive_result}
            }).eq('sermon_id', sermon_id).eq('task_type', 'video_processing').execute()
        ...
        _handle_task_failure(supabase, sermon_id, 'video_processing', e, self.request.retries)
        raise
...
def extract_gps_location(self, sermon_id: str, assigned_user: Optional[str] = None):
    ...
    try:
        ...
        _handle_task_failure(supabase, sermon_id, 'location_tagging', e, self.request.retries)
        raise
...
def analyze_sermon_metadata(self, sermon_id: str, assigned_user: Optional[str] = None):
    ...
    try:
        ...
        # Get transcript
        transcript = supabase.table('sermon_transcripts').select('*').eq(
            'sermon_id', sermon_id
...
- suggested_tags: Array of 5-10 searchable tags"""
        ...
        _handle_task_failure(supabase, sermon_id, 'metadata_ai', e, self.request.retries)
        raise
    ...
    try:
        if supabase and assigned_user:
            supabase.table('sermon_tasks').update({
                'status': 'in_progress',
                'started_at': datetime.now(timezone.utc).isoformat()
            }).eq('sermon_id', sermon_id).eq('task_type', 'quality_optimization').execute()
        ...
        _handle_task_failure(supabase, sermon_id, 'quality_optimization', e, self.request.retries)
        raise
    ...
    try:
        if supabase and assigned_user:
            supabase.table('sermon_tasks').update({
                'status': 'in_progress',
                'started_at': datetime.now(timezone.utc).isoformat()
            }).eq('sermon_id', sermon_id).eq('task_type', 'thumbnail_generation').execute()
        ...
        _handle_task_failure(supabase, sermon_id, 'thumbnail_generation', e, self.request.retries)
        raise
...
def create_social_clips(self, sermon_id: str, assigned_user: Optional[str] = None):
    ...
    try:
        ...
        _handle_task_failure(supabase, sermon_id, 'social_clip', e, self.request.retries)
        raise
...
def auto_sort_sermon(self, sermon_id: str, metadata: Dict[str, Any]):
    ...
    try:
        ...
        if series_title and supabase:
            ...
            series = supabase.table('sermon_series').select('id').ilike(
                'title', series_title
            ).execute()
            
            if not series.data:
                # Create new series
                new_series = supabase.table('sermon_series').insert({
                    'title': series_title,
                    'church_id': supabase.table('sermons').select('church_id').eq(
                        'id', sermon_id
                    ).execute().data[0].get('church_id')
                }).execute()
...
def finalize_sermon_pipeline(self, results: List[Dict], sermon_id: str):
    ...
    try:
        ...
        tasks = supabase.table('sermon_tasks').select('status').eq(
            'sermon_id', sermon_id
        ).execute()
...
# ==================== Helper Functions ====================
def _handle_task_failure(supabase, sermon_id: str, task_type: str, 
...
def cleanup_stale_tasks():
    ...
    supabase.table('sermon_tasks').update({
        ...
        'started_at', cutoff.timestamp() - 7200
    ).execute()
...
def retry_failed_tasks():
    ...
    failed_tasks = supabase.table('sermon_tasks').select('*').eq(
        'status', 'failed'
    ).lte('retry_count', 2).execute()
