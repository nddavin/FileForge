...
class PreacherDataset(Dataset):
    ...
    def __init__(self, data_dir: str, sample_rate: int = 16000, 
                 transform=None, label_encoder: Optional[LabelEncoder] = None):
...
class ECAPAClassifier(nn.Module):
    ...
    def __init__(self, num_classes: int, pretrained_source: str = "speechbrain/spkrec-ecapa-voxceleb"):
        super().__init__()
        ...
        try:
            ...
            emb_dim = self.encoder.modules.encoder.emb.shape[1] if hasattr(self.encoder.modules.encoder, 'emb') else 192
...
class PreacherVoiceTrainer:
    ...
    def evaluate(self, test_dataset: Dataset) -> Dict:
        ...
        return {
            ...
            'predictions': list(zip(all_names, 
                                   [self.label_encoder.classes_[p] for p in all_preds]))
        }
...
class ContinuousLearningTrainer:
    ...
    def collect_training_data(self, min_confidence: float = 0.9, 
                              limit_per_preacher: int = 50) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            for row in result.data:
                if row.get('audio_path') and row.get('primary_preacher_id'):
                    # Limit per preacher
                    preacher_files = [f for l, f in zip(labels, files) 
    ...
    def fine_tune(self, files: List[str], labels: List[str], 
                  epochs: int = 10) -> Dict:
        ...
        history = self.trainer.train(
            PreacherDataset("backend/data/preacher_voices"),  # Base data
            epochs=epochs
        )
...
def evaluate_preacher_model(
    model_path: str,
    test_data_dir: str = "backend/data/preacher_voices"
) -> Dict:
