...
class TestSermonPipelineIntegration:
    ...
    async def test_complete_pipeline_with_gps_and_speaker_id(
        ...
        # Mock GPS extraction
        with patch('backend.file_processor.services.gps_extractor.AudioGPSExtractor') as MockGPS:
            gps_instance = MagicMock()
            gps_instance.extract_gps_from_audio = AsyncMock(return_value=mock_gps_result)
            MockGPS.return_value = gps_instance
            
            # Mock speaker identification
            with patch('backend.file_processor.services.speaker_identifier.SpeakerIdentifier') as MockSpeaker:
                speaker_instance = MagicMock()
                speaker_instance.identify_speakers = AsyncMock(return_value=mock_speaker_result)
                ...
                assert result['gps_location']['readable_location'] == 'Kampala Central Church'
                assert result['speaker_identification']['preacher']['name'] == 'Pastor John'
    ...
    @pytest.mark.asyncio
    async def test_pipeline_handles_speaker_id_failure(self, test_file, mock_gps_result):
...
class TestBulkOperations:
    ...
    def test_bulk_sort_applies_rules(self):
        ...
        sort_rules = [
            {'field': 'preacher_id', 'value': 'pastor-john', 'target_folder': '/sermons/john'}
        ]
        ...
        john_files = [f for f in sorted_files if f['predicted_folder'] == '/sermons/john']
        assert len(john_files) == 2
        
        unsorted_files = [f for f in sorted_files if f['predicted_folder'] == '/unsorted']
