...
class TestRBACEndpoints:
    ...
    @pytest.mark.parametrize("role,token_fixture", [
        ("admin", "admin_token"),
        ("manager", "manager_token"),
        ("user", "user_token"),
    ])
    def test_endpoint_role_access(
        self, test_case, role, token_fixture, request
    ):
...
class TestUnauthenticatedAccess:
    ...
    def test_no_token_returns_401(self, test_case):
        ...
        is_protected = len(test_case["allowed_roles"]) < 3 or "admin" in test_case["allowed_roles"]
...
def test_rbac_compliance_matrix():
    ...
    for test_case in ENDPOINT_TESTS:
        assert len(test_case["allowed_roles"]) > 0, (
    ...
    admin_access_count = sum(
        1 for r in results 
        if ":admin" in r and results[r]["allowed"]
    )
    manager_access_count = sum(
        1 for r in results 
        if ":manager" in r and results[r]["allowed"]
    )
    user_access_count = sum(
        1 for r in results 
        if ":user" in r and results[r]["allowed"]
    )
